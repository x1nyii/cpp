...
//This application allows our Storage Company to provide crates and storage to Ore Companies
...
public:
    ...
    // better to use enum is this case for contents type as it is standard/fixed

    /// <summary>There are 3 sizes for each crate type. Small (800kg) , Medium (1600 kg) and Large (2400 kg).</summary>
    ...
    /// <summary>Crates are specifically tailored to contain pre-determined contents: Toxic waste, General Waste, Ore, or Refined Ore</summary>
    enum class ContentsType
    {
        ...
        ORE,
        REFINED_ORE
    }; // use enum class and cast it to integer to string it below

    ContentsType contentsType; // Use a different name for the member variable to avoid confusion

    /// <summary>Function CrateBase to store the base variables of a crate</summary>
    ...
    //CrateBase(string clientName, string mineralName, int weight)
    //    : ClientName(clientName), MineralName(mineralName), Weight(weight) {}
    ...
    virtual void PrintContainerDetails()
    {
        ...
        switch (content)
        {
            ...
            case 3: cout << "Mineral type = Refined Ore" << endl;
                break;
            default: cout << "Unknown Content Type"; 
                break;
        }
    }
};
...
// ContentsType content;
// 
//        Crate(string colour, string mineralName, int weight) : CrateBase(mineralName, )
//            cout << "Crate created.\n";
//}
    ...
    if (IsFileExist("records.dat"))
    {
        ...
        for (size_t i = 0; i < lines.size(); i += 4)
        {
            ...
            if (newCrate != nullptr)
            {
                ...
                newCrate->contentsType = static_cast<CrateBase::ContentsType>(stoi(lines[i + 3]));
...
/// <summary>Write all client records to file</summary>
/// <param name="CrateList">List of all the crates</param>
void WriteRecordsToFile(vector<CrateBase> CrateList)
{
    ...
    for (CrateBase& crate : CrateList)
    {
        ...
        // contents.push_back(ReadRecordsFromFile(crate.contentsType));
...
int main()
{
    ...
    do
    {
        ...
        switch (option)
        {
            ...
            case 2: //Add a record
                {
                    cout << "Please select how many minerals to store in this crate: " << endl;
                    ...
                    {
                        cout << "Invalid selection. Please select either 1 or 2 minerals in a crate." << endl;
                        return;
                    }

                    // Store all the 16 minerals as strings in a vector 
                    vector <string> minerals = 
                    { 
                        "Iron", "Maganese", "Lead", "Gold", "Copper", "Aluminium", "Chromite", "Cinnabar", " Cinnabar+Chromite", "Beryl", "Galena", "Tin", 
                        "Malachite", "Uranium", "Barite", "Lead+Maganese" // 4 new minerals
                    };
                        
                    // display a list of all the minerals
                    cout << "Please select the mineral option: " << endl;
                    for (int i = 0; i < minerals.size(); ++i)
                    {
                        cout << i + i << ". " << minerals[i] << endl;
                    }
                    ...
                    newCrate->ContentsType = static_cast<CrateBase::ContentsType>(static_cast<int>(newCrate->contentsType) - 1;
                    cout << endl;
